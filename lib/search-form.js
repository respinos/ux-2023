...
export class HathiSearchForm extends LitElement {
  ...
  firstUpdated() {
    ...
    if ( location.host.indexOf('babel.hathitrust.org') > -1 ) {
      if ( location.host.indexOf('babel.') > 0 ) {
        let prefix = (location.host.split('.'))[0];
        this.SERVICE_DOMAIN = `${prefix}.${this.SERVICE_DOMAIN}`;
        this.CATALOG_DOMAIN = `${prefix}.${this.CATALOG_DOMAIN}`;
      }
    }
